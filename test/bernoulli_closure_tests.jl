...
@test length(unknowns(closed_eqs.odes)) == 4 && isequal(closed_eqs.closure[M[0,4]], 0)

closed_eqs = moment_closure(sys, "normal", binary_vars)
@test length(unknowns(closed_eqs.odes)) == 4 && isequal(closed_eqs.closure[M[0,4]], 3*M[0,2]^2)
...
@test length(unknowns(closed_eqs.odes)) == 4 && isequal(simplify(expr1), simplify(expr2))

closed_eqs = moment_closure(sys, "poisson", binary_vars)
@test length(unknowns(closed_eqs.odes)) == 4 && isequal(closed_eqs.closure[M[0,3]], μ[0,1])

closed_eqs = moment_closure(sys, "gamma", binary_vars)
expr1 = closed_eqs.closure[M[0,3]]
expr2 = 2*M[0,2]^2*μ[0,1]^-1
@test length(unknowns(closed_eqs.odes)) == 4 && isequal(expr1, expr2)
...
closed_eqs = moment_closure(sys, "zero", binary_vars)
@test length(unknowns(closed_eqs.odes)) == 4
