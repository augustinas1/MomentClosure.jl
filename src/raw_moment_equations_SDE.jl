function generate_raw_moment_eqs(
    ...
    for iter in iter_m
        ...
        for i in 1:N
            ...
            gradh = iter .- iter_1[i]
            term1 += gradf * sum(
                μ[gradh .+ Tuple(drift_powers[i][j])] * drift_factors[i][j]
        ...
        for i in 1:N
            for j in 1:N
                ...
                term2 += hji_factor * sum(
                    μ[hji .+ Tuple(diff_powers[i, j][k])] * diff_factors[i, j][k]
    ...
    return RawMomentEquations(odes, smap, μ, N, m_order, q_order, iter_all, iter_m, iter_q, iter_1)
end
...
"""
function generate_raw_moment_eqs(sys::System, m_order::Int)
    iv = get_iv(sys)
    ...
    return generate_raw_moment_eqs(
        equations(sys), noise_eqs, m_order,
