...
function expected_coeff_div(x, n::Int)
    ...
    return (expected_coeff(num, n) / expected_coeff(denom, n))
end

function expected_coeff_term(x, n::Int)
    return if isequal(operation(x), rand)
        args = arguments(x)
...
function resolve_moment(d, args, n)
    ...
    return geometric_raw_moment(args[1], n)
end
...
function geometric_raw_moment(p, n::Int)
    ...
    return simplify(expand(p^(-n) * suma))
end
